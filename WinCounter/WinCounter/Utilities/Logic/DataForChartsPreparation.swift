...
class DataForChartsPreparation {
    ...
    static func dataForEachPlayersChartsPreparation(player: Player) -> [DataForEachPlayer] {
        ...
        for rival in StatisticPreparation.makingRivalsArray(for: player) {
            
            var dataStructuresDictionary = [DataStructuresForChart]()
            
            let wins = StatisticPreparation.calculateWins(for: player, with: rival)
            let winValue = DataStructuresForChart(label: "Wins", value: wins)
            dataStructuresDictionary.append(winValue)
            
            let losses = StatisticPreparation.calculateLosses(for: player, with: rival)
    ...
    var data = [DataStructuresForChart]()
        let value1 = DataStructuresForChart(label: "Total wins", value: StatisticPreparation.totalWins(for: player))
        let value2 = DataStructuresForChart(label: "Total losses", value: StatisticPreparation.totalLosses(for: player))

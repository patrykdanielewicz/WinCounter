        ...
        VStack {
            ForEach(dataForEachPlayersCharts) { chart in
                HStack {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        .chartBackground { chartProxy  in
                            GeometryReader { geometry in
                                ...
                                VStack {
                                    Text("\(StatisticPreperation.calculateWins(for: player, with: chart.name))")
                                        .font(.title)
                                        .fontWeight(.heavy)
                                    Text("matches")
                                        .font(.footnote)
                    ...
                    VStack {
                         let win = StatisticPreperation.calculateWins(for: player, with: chart.name)
                         let losses = StatisticPreperation.calculateLosses(for: player, with: chart.name)
